...
const detalle = document.getElementById("detalle") as HTMLElement | null;

function hacerCard(serie: Serie): void {
  if (!detalle) return;
  detalle.innerHTML = `
...
function hacerTabla(filas: HTMLTableRowElement[]) {
  if (!body) throw new Error("No se encontró <tbody id='elbody'> en el HTML");
  body.innerHTML = "";                                 
